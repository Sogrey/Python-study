...
def find_main_charecters(charecter_name):
    with open('./test_1st/sanguo.txt', encoding='gb18030') as f:
        data = f.read().replace("\n", "")
...
name_dict = {}
with open('./test_1st/name.txt', encoding='gb18030') as f:
    for line in f:
        names = line.split('|')
        for n in names:
...
weapon_dict = {}
with open('./test_1st/weapon.txt', encoding='gb18030') as f:
    i = 1
    for line in f:
        if i % 2 == 1:
